...
from generated.grpc.actr import NeuroAction
...
class ProceduralMemory(ProceduralMemoryServiceBase):
    ...
    async def get_all_conditions(self, betterproto_lib_google_protobuf_empty) -> GetAllConditionsResponse:
        ...
        response = GetAllConditionsResponse()
        return response
    ...
    async def select_rule(self, select_rule_request: SelectRuleRequest) -> NeuroAction:
        ...
        applicable = [r for r in self.rules.values()
                      if r.condition_fn is not None and r.condition_fn(select_rule_request)]
        if not applicable:
        ...
        _ = random.choices(applicable, weights=probs, k=1)[0]
